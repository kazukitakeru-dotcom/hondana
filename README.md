...
- **シリーズ** — 漫画などをシリーズとしてまとめられる。本棚にはシリーズが1枚のカードで並び、
  **タップすると棚の上でそのまま全巻が表紙で並ぶ**（もう一度タップで畳む。開いた状態は覚えている）。
  巻は**横スクロールの帯**で出るので、100巻あっても縦に伸びて他の本が押し流されることはない。
  巻の管理（追加・まとめて読了など）は表紙左上の ⚙ から。
  管理画面では巻が 1 2 3 … と並び、持っていない巻は点線で出るのでタップで足せる。
  上には「1〜4, 6〜8巻を持っています／次に買うのは5巻」という要約が出る。
...
- **検索・絞り込み** — タイトル・著者・タグ・メモ・ISBN・棚名・電子ストアをまとめて検索。
  大文字小文字や空白の違い（「尾田 栄一郎」と「尾田栄一郎」）、全角半角、ひらがなカタカナを
  区別しない。空白で区切ると「全部を含む」で絞れる。状態・棚・タグ・お気に入りでのフィルタも可能。
  **読みでも引ける** — 「おだ」で尾田栄一郎、「わがはい」で吾輩は猫である。
  設定の「読みがなを取得」を押すと、すでに登録済みの本にも読みが入る
  （ISBNのある本が対象。ASCIIの題名は読みのデータが無いため「ワンピース」では引けない）
