...
「シリーズを作れるのに、そこへ追加する手段が無く、数字を打ち込むのは変な構造」と
指摘されて足した導線。巻ごとに1冊ずつ登録すると棚が埋まるので、集約する形は変えていない。

`offerSeriesAction()` が読み取った題名を `splitTitleVolume()` で作品名と巻数に分け、

- 同じ作品のシリーズが既にある → **「N巻を追加」**（`addVolumeToSeries()` で1タップ）
- まだ無い → **「シリーズにする」**（題名を作品名だけにして `volumes` にその巻を入れる）

これで「1巻を読み取ってシリーズにする → 以降は読み取るたびに1タップで足す」が回る。
シリーズの題名を**巻数を外した作品名**にするのは、次に読み取ったときに名前で照合するため。

編集画面の **「＋次の巻」**（`addNextVolumeBtn`）は、抜けがあればそこを先に埋め、
無ければ続きの巻を足す。全巻そろっていれば何もしない。
